...
static inline bool check_lba_range(uint64_t sector_num, uint32_t nb_sectors)
{
    /*
...
static void scsi_unmap_complete(void *opaque, int ret)
{
    ...
    if (data->count > 0 && !r->req.io_canceled) {
        ...
        if (!check_lba_range(sector_num, nb_sectors)) {
...
static int32_t scsi_disk_emulate_command(SCSIRequest *req, uint8_t *buf)
{
    ...
    case WRITE_SAME_10:
    case WRITE_SAME_16:
        nb_sectors = scsi_req_cdb_length(r->req.cmd.buf);
        if (bdrv_is_read_only(s->qdev.conf.bs)) {
            scsi_check_condition(r, SENSE_CODE(WRITE_PROTECTED));
            return 0;
        }
        if (!check_lba_range(r->req.cmd.lba, nb_sectors)) {
            goto illegal_lba;
        }
...
static int32_t scsi_disk_dma_command(SCSIRequest *req, uint8_t *buf)
{
    ...
    len = scsi_req_cdb_length(r->req.cmd.buf);
    switch (command) {
    case READ_6:
    case READ_10:
    case READ_12:
    case READ_16:
        ...
        if (!check_lba_range(r->req.cmd.lba, len)) {
            goto illegal_lba;
        }
